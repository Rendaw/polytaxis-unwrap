...
size_t GetHeaderLength(std::string const &Path)
{
	...
	try
	{
		...
		if (Length < 0)
		{
			...
			std::string Parts[2];
			int Next = 0;
			for (int Next = 0; File.Read(Buffer); Next = !Next)
			{
...
size_t WriteEmptyHeader(std::string const &Path)
{
	auto File = Filesystem::FileT::OpenWrite(Path);
	File.Write("polytaxis00 00000000512\n\0");
}
...
struct FilesystemT
{
	int create(bool const OutOfBand, const char *path, mode_t mode, struct fuse_file_info *fi)
	{
		...
		SetFile(fi, std::make_shared<FileT>({fd, Length}));
		return 0;
	}
	...
	int open(bool const OutOfBand, const char *path, struct fuse_file_info *fi)
	{
		...
		if (fd == -1)
			return -errno;
		SetFile(fi, std::make_shared({fd, GetHeaderLength(Path)}));
		return 0;
	}
	...
	int read(bool const OutOfBand, const char *path, char *out, size_t count, off_t start, struct fuse_file_info *fi)
	{
		...
		auto res = pread(File->Handle, buf, size, File->HeaderLength + offset);
		if (res == -1)
			res = -errno;
		return res;
	}
	...
	int write(bool const OutOfBand, const char *path, const char *out, size_t count, off_t start, struct fuse_file_info *fi)
	{
		Assert(!OutOfBand);
		auto File = GetFile(fi);
		auto res = pwrite(File->Handle, buf, size, File->HeaderLength + offset);
		if (res == -1)
			res = -errno;
		return res;
	}
...
int main(int argc, char **argv)
{
	try
	{
		...
		SignalHandlers.push_back([&Shared](void) { Shared.Fuse.Kill(); });
